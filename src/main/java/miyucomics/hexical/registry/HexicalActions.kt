...
object HexicalActions {
	fun init(event: RegisterEvent) {
		...
		register(event, "periwinkle", "wwwaqdadaadadqqqeaeq", HexDir.EAST, Action.makeConstantOp(IdentifierIota(HexicalMain.id("periwinkle"))))
		...
		register(event, "wristpocket_item", "aaqqada", HexDir.WEST, OpGetWristpocketData { stack -> if (stack.isEmpty) listOf(NullIota()) else stack.asActionResult })
		...
		register(event, "egg", "qqqwaqaaqeeewdedde", HexDir.SOUTH_EAST, OpConjureProjectile(MediaConstants.DUST_UNIT * 2) { world, position, caster ->
			val egg = ThrownEgg(world, position.x, position.y, position.z)
			egg.owner = caster
			return@OpConjureProjectile egg
		})
		register(event, "llama_spit", "dwqaqw", HexDir.EAST, OpConjureProjectile(MediaConstants.DUST_UNIT / 4) { world, position, caster ->
			val spit = LlamaSpit(EntityType.LLAMA_SPIT, world)
			spit.setPos(position)
			spit.owner = caster
			return@OpConjureProjectile spit
		})
		register(event, "snowball", "ddeeeeewd", HexDir.NORTH_EAST, OpConjureProjectile(MediaConstants.DUST_UNIT / 2) { world, position, caster ->
			val snowball = Snowball(world, position.x, position.y, position.z)
			snowball.owner = caster
			return@OpConjureProjectile snowball
		})
		register(event, "ghast_fireball", "wqqqqqwaeaeaeaeae", HexDir.SOUTH_EAST, OpConjureProjectile(MediaConstants.DUST_UNIT * 3) { world, position, caster ->
		...
		register(event, "block_ping", "dwwdwwdwewdwwdwwdeq", HexDir.NORTH_EAST, OpBlockPing())
		register(event, "crack_device", "wwaqqqqqeqdedwqeaeqwdedwqeaeq", HexDir.EAST, OpCrackDevice())
		register(event, "place_block_type", "wewewewewewdwew", HexDir.NORTH_WEST, OpConstruct())
		...
		register(event, "conjure_compass", "aqwawqwqqwqwq", HexDir.SOUTH_WEST, OpConjureCompass())
		...
		register(event, "hopper_in", "qwawqwaeqqq", HexDir.SOUTH_EAST, OpHopperInsert())
		register(event, "hopper_out", "qqqeawqwawq", HexDir.SOUTH_WEST, OpHopperExtract())
