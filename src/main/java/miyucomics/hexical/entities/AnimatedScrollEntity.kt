...
class AnimatedScrollEntity(entityType: EntityType<AnimatedScrollEntity>, world: Level) : HangingEntity(entityType, world), ADIotaHolder {
	...
	fun setState(state: Int) = this.entityData.set(stateDataTracker, state)
	fun setColor(color: Int) = this.entityData.set(colorDataTracker, color)
	fun toggleGlow() = this.entityData.set(glowDataTracker, !this.entityData.get(glowDataTracker))
